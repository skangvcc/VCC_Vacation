...
st.markdown("""
<style>
    .app-title { font-size: 26px; font-weight: bold; color: #333; margin-top: 5px; margin-bottom: 20px; }
    ...
    .card-value { font-size: 28px; font-weight: bold; color: #E74C3C; } /* 불꽃 색상과 맞춤 */
...
# --- 5. 상단 UI: 불꽃 심볼 로고 적용 ---
# 사이드바나 로컬 경로에 로고 이미지가 없다면 웹 주소를 사용합니다.
symbol_url = "https://githubusercontent.com" # 예시 주소, 실제 주소로 변경 가능
st.image(symbol_url, width=80) 
st.markdown('<p class="app-title">본한인교회 휴가 관리 시스템</p>', unsafe_allow_html=True)
...
if st.session_state.menu_sel == "현황":
    ...
    for idx, row in st.session_state.staff.iterrows():
        ...
        sc2.markdown(f'<p class="usage-text">사용 {int(used)}일 / 한도 {int(total_limit)}일</p>', unsafe_allow_html=True)
        ...
        sc4.markdown(f'<p class="rem-text">잔여<br><span style="font-size:18px; color:#E74C3C;">{int(rem)}일</span></p>', unsafe_allow_html=True)
    ...
    if pw == st.session_state.admin_password:
        ...
        if st.button("저장"):
            ...
            st.success("저장 완료!")
            st.rerun()
    else: st.warning("비밀번호를 입력하세요.")

elif st.session_state.menu_sel == "신청":
    st.title("📝 휴가 신청")
    with st.form("leave_form"):
        name = st.selectbox("성함", st.session_state.staff['Name'])
