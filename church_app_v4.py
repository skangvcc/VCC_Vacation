...
# --- 5. 로직: 주말 및 실시간 공휴일 라이브러리 기반 일수 제외 계산 ---
def calculate_church_days(start_date, end_date, l_type):
    ...
    curr = start_date
    while curr <= end_date:
        # 해당 날짜가 속한 연도의 공휴일 동적으로 가져오기
        yearly_holidays = get_ontario_holidays(curr.year)
        
        is_weekend = curr.weekday() >= 5
        is_holiday = curr.strftime("%Y-%m-%d") in yearly_holidays
